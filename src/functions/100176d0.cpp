...
extern "C" int __cdecl __isctype_l(int, int, localeinfo_struct*);

extern "C" int __cdecl __isxdigit_l(int c, void* locale)
{
    ...
    if (locale_name_length <= 1)
    {
        ...
        result = *reinterpret_cast<const std::uint16_t*>(
            ctype_table + static_cast<std::intptr_t>(c) * 2) & 0x80U;
    }
    else
    {
        result = static_cast<std::uint32_t>(
            __isctype_l(c, 0x80, reinterpret_cast<localeinfo_struct*>(local)));
    }
