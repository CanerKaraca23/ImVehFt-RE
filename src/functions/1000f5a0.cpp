...
extern std::int32_t DAT_1003c3d0;

using Callback = void (__cdecl*)(std::uint32_t);
using ReturnCallback = std::uint32_t (__cdecl*)();

std::uint32_t __stdcall FUN_1000f5a0(std::uint32_t param_1)
{
    const std::int32_t iVar2 = DAT_1003c3d0;

    auto* puVar1 = *reinterpret_cast<std::uint32_t**>(
        static_cast<std::uintptr_t>(DAT_1003c3d0) + 0x1c
    );

    for (auto* puVar3 = *reinterpret_cast<std::uint32_t**>(
             static_cast<std::uintptr_t>(DAT_1003c3d0) + 0x18);
         puVar3 != puVar1;
         puVar3 += 1)
    {
        if (*puVar3 != 0U)
    ...
    const auto callback = *reinterpret_cast<ReturnCallback*>(
        static_cast<std::uintptr_t>(iVar2) + 0x0c
    );
