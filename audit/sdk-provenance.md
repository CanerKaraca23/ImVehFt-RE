...
The resulting `Plugin2014.lib` was added to the same strict 705-object diagnostic link used with the modern SDK. Before address shims, both probes reported 228 unresolved externals and produced no DLL. The 2026-09-24 follow-up probe adds only diagnostic wrappers for functions whose fixed addresses/signatures are documented in Plugin-SDK's `RenderWare.cpp`; it reaches 210 unresolved externals, still with no DLL. This is a symbol-resolution experiment, not proof that the SDK vintage or inferred call behavior matches the original build. See [`link-probe-2026-09-24.md`](link-probe-2026-09-24.md).
