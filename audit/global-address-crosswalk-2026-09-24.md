...
| `0x00C3EF78` | `CharsetLockedData`, an unsigned-byte/data pointer | Candidate declares it as `Texture*` in `10003ba0.cpp`, stores the return of the raster-lock call, then later uses the value as a byte-buffer base in `10003e60.cpp`. The data-flow use is consistent with a locked pixel-data pointer despite the misleading candidate type. |
